...
const addPost = async (msg, type) => {
  ...
  let nameUser = msg.chat.username || "Користувач";
  try {
    const now = new Date();
...
bot.on("message", async (msg) => {
  ...
  switch (msg.text) {
    ...
    case keyboardBtns.home.my:
      ...
      await getMyPosts(chatId, { number: msg.from.number });
      break;
...
bot.on("callback_query", async (query) => {
  ...
  switch (type) {
    case ACTION_TYPE.TOGGLE_FAV_POST:
      await toggleFavPost(userId, query.id, data);
      ...
      await getFavPosts(chatId, query);
    ...
    case ACTION_TYPE.GET_POSTS_BY_USER:
      ...
      await getPosts(chatId, { authorId: data.authorId });
    ...
    case ACTION_TYPE.DELETE_POST:
      ...
      getMyPosts(chatId, { number: query.message.chat.number });
